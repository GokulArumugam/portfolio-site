...
- **No real login yet.** The API trusts the key store, but there's no SSO/OAuth in front of it. The security *ideas* (firewall, scoping, audit) are real and tested; the *auth layer* around them is deliberately left as "later."
...
Grab the code at [github.com/GokulArumugam/atlas](https://github.com/GokulArumugam/atlas) — `docker compose up --build` and you're off.
